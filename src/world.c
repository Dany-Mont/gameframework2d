...
World *world_test_new()
{
    ...
    world->tileSet = gf2d_sprite_load_all("../images/backgrounds/tileset.png",16,16,1,1);
    for (i=0; i < width;i++)
    {
        world ->tileMap[i] = 1;
    ...
    for (j=0; j < height;j++)
    {
        world ->tileMap[j*width] = 1;
        world ->tileMap[j*width + (width-1)] = 1;
    }
...
World *world_new(Uint32 width, Uint32 height)
{
    ...
    if(!width || !height)
    {
        slog("cannot create world with zero width or height");
        return NULL;
    }
...
{
    if(!world)return;
    gf2d_sprite_free(world->background);     //background image for the world
    gf2d_sprite_free(world->tileSet);            //sprite containing the tiles to draw with
    gf2d_sprite_free(world->tileLayer);        //prerendered tile layer for the world
    free(world->tileMap);   
    free(world); 
}
...
World *world_load(const char *filename)
{
    ...
    if(!json)
        {
            slog("failed to load world from file: %s",filename);
            return NULL;
        }
    wjson = sj_object_get_value(json,"world");
    if (!wjson)
        {
            slog("failed to find world data in json");
            sj_free(json);
            return NULL;
        }
    ...
    if(!vertical)
        {
            slog("failed to find tileMap data in json");
            sj_free(json);
            return NULL;
        }
